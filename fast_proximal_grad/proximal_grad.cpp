...
inline __m256 avx_sign_ps(const __m256 x)
{
    const __m256 zero = _mm256_setzero_ps();

    __m256 positive = _mm256_and_ps(_mm256_cmp_ps(x, zero, AVX_CMP_GT), _mm256_set1_ps(1.0f));
    __m256 negative = _mm256_and_ps(_mm256_cmp_ps(x, zero, AVX_CMP_LT), _mm256_set1_ps(-1.0f));
...
inline __m256 avx_abs_ps(const __m256 x)
{
	const __m256 zero = _mm256_setzero_ps();
